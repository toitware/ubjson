...
func BenchmarkEncodingJsonStructSmallMarshal(b *testing.B) {
	var st SmallPayload
	json.Unmarshal(smallFixture, &st)
...
*/
func BenchmarkJmank88UbjsonSmall(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var data SmallPayload
		jmank_ubjson.Unmarshal(smallFixtureUBJSON, &data)

		nothing(data.Uuid, data.Tz, data.Ua, data.St)
	}
}

func BenchmarkJmank88UbjsonSmallMarshal(b *testing.B) {
	var st SmallPayload
	jmank_ubjson.Unmarshal(smallFixtureUBJSON, &st)

	for i := 0; i < b.N; i++ {
		data, _ := jmank_ubjson.Marshal(st)

		nothing(data)
	}
}

func BenchmarkToitwareUbjsonSmall(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var data SmallPayload
		ubjson.Unmarshal(smallFixtureUBJSON, &data)

		nothing(data.Uuid, data.Tz, data.Ua, data.St)
	}
}
func BenchmarkToitwareUbjsonSmallMarshal(b *testing.B) {
	var st SmallPayload
	ubjson.Unmarshal(smallFixtureUBJSON, &st)
